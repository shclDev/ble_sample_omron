...
public class SessionFragment extends BaseFragment implements
    ...
    public void onBluetoothStateChanged(boolean enable) {
        mTerminalSettingsStage = TerminalSettingsStage.BLUETOOTH_CHECK;
        checkSettingsState(mBluetoothPowerController.state());
    }
    ...
    public void onSessionComplete(@NonNull SessionData sessionData) {
        ...
        ResultType sessionResult = validateSessionWithData(mHistoryData.getProtocol(), sessionData, mHistoryData.getComType());
    ...
    @NonNull
    private ResultType validateSessionWithData(@NonNull Protocol protocol, @NonNull SessionData sessionData, ComType type) {
        ...
        if (null == sessionData.getBatteryLevel()
                && null == sessionData.getCurrentTime()
                && null == sessionData.getMeasurementRecords()) {
            return ResultType.Failure;
        }
        ...
        if(OHQDeviceCategory.PulseOximeter == sessionData.getDeviceCategory()
                && 0 >= sessionData.getMeasurementRecords().size()
                && type == ComType.Transfer) {
            return ResultType.Failure;
        }
